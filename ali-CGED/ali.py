...
("他 们 是 不 但 我 父 母 ， 而 且 是 人 生 的 先 辈 。 ".split(),"O O O B-W I-W I-W O O O O O O O O O O B-S I-S O ".split()),
...
'''
tag_to_ix = {"O": 0, "B-R": 1, "I-R": 2,"B-M": 3, "I-M": 4, "B-S": 5, "I-S": 6,"B-D": 7, "I-D": 8, START_TAG: 9, STOP_TAG: 10}
...
for epoch in range(300):  # again, normally you would NOT do 300 epochs, it is toy data
    ...
    for i in range(len(feat_part1_without_dependence)):
        ...
        tags = labels[i].replace("\n","").split(',')
        targets = torch.LongTensor([tag_to_ix[t] for t in tags])
        if use_cuda:
            sentence_in1, sentence_in2, targets = sentence_in1.cuda(), sentence_in2.cuda(), targets.cuda()
        # Step 3. Run our forward pass.
        neg_log_likelihood = model.neg_log_likelihood(sentence_in1,sentence_in2,targets)
        sumscores = sumscores+neg_log_likelihood.data
        ...
        optimizer.step()
    print(sumscores/len(feat_part1_without_dependence))
...
sentence_in1 = feat_part1_without_dependence[0]
sentence_in2 = feat_part2_dependence[0]
sentence_in1 = Variable(torch.from_numpy(np.array(sentence_in1))).float()
sentence_in2 = Variable(torch.from_numpy(np.array(sentence_in2))).float()
